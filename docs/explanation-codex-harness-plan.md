...
역할의 책임·핵심 금지·관련 문서는 [공식 역할 TOML](../.codex/agents/), 공통 협업은
[AGENTS.md](../AGENTS.md), 상세 프로젝트 사실은 기존 플랫폼·상태·API 문서에서 관리한다.
구현 순서나 판단 방법을 고정하거나 같은 설명을 에이전트 전용 문서에 복제하지 않는다.
...
Docker 부재로 생략된 DB 통합 테스트와 미수행 검증을 성공으로 보고하지 않는다.

...
| [MAST: Why Do Multi-Agent LLM Systems Fail?](https://arxiv.org/html/2503.13657v3) | 성급한 종료·검증 누락·잘못된 검증을 별개 실패로 보고, 완료 조건과 실행 근거를 연결한다. |
| [CONSENSAGENT](https://aclanthology.org/2025.findings-acl.1141/) | 에이전트 간 동조 위험을 고려해 독립 검토를 두며, 합의만으로 완료를 판정하지 않는다. |
| [Anthropic 에이전트 평가 안내](https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents) | 실제 산출물·환경 상태를 검사하고, 검증자도 정상·결함 사례로 점검한다. |

위 자료의 원칙을 선택해 적용하며, 각 제품의 실행 방식이나 검증 절차를 그대로 도입하지 않는다.

## 책임과 문서의 위치

구현 역할은 Frontend·Desktop·Server 세 개이며, 동작·계약·상태·실행 설정 또는 검증·완료 판정
규칙 변경은 구현에 참여하지 않은 Reviewer가 검토한다. 실제 화면·OS 동작 관찰이 필요한
완료 조건은 Visual QA가 확인한다. 순수 문구·서식·링크 수정은 문서 검사로 확인한다.
...
요구사항 확인·검증 결과 통합·반복 실패 대응의 상세 절차는 [메인 작업 안내](guide-codex-main.md)에 분리한다.
...
역할과 문맥을 나누어도 같은 가정에 동조하거나 검증을 생략할 수 있다. 따라서 모든 작업의 완료
조건에 근거를 요구하고, 코드 검토와 실행 검증의 결과를 구분한다. 필수 조건의 실패·미검증이
남으면 완료로 처리하지 않는다. 수정한 지적과 영향 범위를 재검증하고, 개별 결과를 연결한 뒤에도
최종 조합을 확인한다. 검증자는 결함 개수를 채우거나 근거 없이 반대할 필요가 없다.

이는 에이전트의 작업·보고 지침이며 별도 실행기나 새로운 CI 병합 차단 장치를 도입하지 않는다.
실행 가능한 제약은 기존 검사·CI로 확인하고, 지침을 바꾸면 정상·결함·미검증 사례로 검증자의
행동도 점검한다. 연구의 실험 결과나 소수 사례의 성공을 이 프로젝트의 탐지율로 일반화하지 않는다.
진전 없는 반복은 메인 안내의 중단 기준을 따르며, 모든 에이전트가 동의할 때까지 반복하지 않는다.

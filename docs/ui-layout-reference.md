...
- 수집 확인 시각과 현재 원본 내용의 첫 관측 시각을 구분한다. 원본 내용이 계속 동일한 경우
  수집 실패나 실제 기기 갱신 지연으로 단정하지 않으며, 저장된 응답의 유효성 경고와도
  구분한다. 서버 시각이 PC보다 최대 60초 앞선 응답은 작은 시계 차이로 허용하되,
  응답의 최대 유효 시간과 원본 신뢰도 기준은 유지한다. 신뢰할 수 없는 상태의 시작 가능 수와
  새 알림 등록은 계속 제한한다.

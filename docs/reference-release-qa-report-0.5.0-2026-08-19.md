...
| LMS 종단 간 | 미실행 | 실제 로그인과 학습 시작·학습 종료 상태 변경 미검증 |
...
- 출석 종단 간: `ATT-001`~`ATT-007`, 실제 LMS 로그인·학습 시작·학습 종료 상태 변경·날짜 경계
...
3. 실제 Jungle Campus에서 학습 시작·학습 종료의 미완료와 완료 상태를 전환해 PC → 서버 → PWA 표시를 비교한다.

...
자동 시작·디버그·로그 폴더는 해당 PC에만 영향을 주므로 로컬 설정과 제한된 Tauri IPC가
소유합니다. 자동 업데이트는 사용자 설정이 아니라 Rust coordinator가 항상 관리합니다.
업데이트가 발견되면 대상 버전만 원자적으로 기록하고, 사용자 창이 열린 프로세스에서는
설치와 재시작을 미룹니다. 다음 실행은 endpoint와 서명을 다시 검증한 뒤 설치하며 확인
실패 시 기존 앱을 그대로 엽니다. 자동 설치는 버전당 세 번까지만 시도하고 실패 뒤 한 시간
동안 startup preflight를 건너뛰며, 상한 이후에는 수동 설치만 허용합니다. 사용 통계 선택은
PC 설정 화면에서 편집하지만 연결된 PWA에도 같은 gate가 필요하므로 nullable 로컬 상태를
서버 계정 preference에 동기화합니다.

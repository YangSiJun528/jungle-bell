...
const checker = source('../../desktop/src/checker.rs');
const tray = source('../../desktop/src/tray.rs');
...
test('대시보드는 첫 페인트부터 라이트·다크 color scheme과 테마 색상을 모두 선언한다', () => {
    ...
    assert.match(
        dashboard,
        /<meta name="theme-color" media="\(prefers-color-scheme: dark\)" content="#[0-9a-f]{6}" \/>/i,
    );

...
const checkerSource = readFileSync(new URL('./platform/tauri/checker/checker.ts', srcRoot), 'utf8');
const rootPackage = JSON.parse(readFileSync(new URL('../package.json', srcRoot), 'utf8')) as {
    scripts?: Record<string, string>;
};
const buildSource = readFileSync(new URL('../../desktop/build.rs', srcRoot), 'utf8');
const appSource = readFileSync(new URL('../../desktop/src/lib.rs', srcRoot), 'utf8');
const checkerRuntimeSource = readFileSync(new URL('../../desktop/src/checker.rs', srcRoot), 'utf8');
const tauriConfig = JSON.parse(
    readFileSync(new URL('../../desktop/tauri.conf.json', srcRoot), 'utf8'),
) as {
    app: {security: {freezePrototype?: boolean}};
};
const capabilityDirectory = new URL('../../desktop/capabilities/', srcRoot);
...
test('구형 출석 WebView injection과 capability는 제거한다', () => {
    ...
    assert.equal(existsSync(new URL('../../desktop/capabilities/attendance.json', srcRoot)), false);
});

...
describe('laundryRiskNotice', () => {
    it('safe risk does not produce a visible notice', () => {
        expect(
            laundryRiskNotice({attempts: 10, errors: 1, rate: 10, riskLevel: 'safe'}),
        ).toBeNull();
    });

    it('formats slight risk with retry guidance', () => {
        expect(
            laundryRiskNotice({attempts: 6, errors: 1, rate: 16.7, riskLevel: 'slight'}),
        ).toEqual({
            label: '약간 주의',
    ...
    it('recommends another machine for caution risk', () => {
        expect(laundryRiskNotice({attempts: 5, errors: 3, rate: 60, riskLevel: 'caution'})).toEqual(
            {
                label: '주의',

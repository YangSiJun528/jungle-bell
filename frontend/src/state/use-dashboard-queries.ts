...
export function useCampusManualRefresh(kind: 'laundry' | 'meals') {
    ...
    const queryKey =
        kind === 'laundry' ? laundryQueryContract.queryKey : mealsQueryContract.queryKey;
    return useMutation({
        mutationKey: ['campus', 'manual-refresh', kind],
        mutationFn: () => client.refetchQueries({queryKey, type: 'active'}, {throwOnError: true}),
    });
}
...
export function useRefreshAttendanceMutation() {
    ...
    return useMutation({
        ...
        mutationFn: () => {
            ...
            const desktopSessionReady =
                !platform.capabilities.desktopAccount || serverSessionReady(account.status);
            return runAttendanceRefresh({
                refreshPlatform: platform.capabilities.desktopAccount
                    ? async () => {
                          try {
...
export function useNotificationsQuery() {
    ...
    return useQuery<DashboardNotification[] | NotificationInboxSnapshot>({
        ...
        queryFn: () =>
            platform.capabilities.localNotifications
    ...
    return useMutation({
        ...
        mutationFn: () =>
            runDashboardRefresh({
                ...
                refreshPlatform: refreshDesktopPlatform
                    ? async () => {
                          try {
                              await api.refreshPlatformSync();
                          } finally {
    ...
    return useMutation({
        mutationFn: async () => {
            await runDashboardRefresh({
                refreshLaundry: () =>
                    client.invalidateQueries({queryKey: laundryQueryContract.queryKey}),
                refreshMeals: () =>
                    client.invalidateQueries({queryKey: mealsQueryContract.queryKey}),
                refreshPlatform: refreshDesktopPlatform
                    ? async () => {
                          try {
                              await api.refreshPlatformSync();

    ...
    return (
        <figure
            className={cn(
                ...
                phone ? 'min-h-[22rem] rounded-[2rem] border-[0.375rem] border-[#272d28]' : 'rounded-2xl',
                className,
            )}
        >
            ...
            <span className="absolute left-1/2 top-2 h-4 w-16 -translate-x-1/2 rounded-full bg-[#252b26]" aria-hidden="true"/>
            <div className="grid justify-items-center pt-10 text-white [text-shadow:0_2px_8px_rgba(26,55,32,.25)]">
                ...
                <time className="text-5xl font-light tracking-[-0.06em]" dateTime="17:28">5:28</time>
            </div>
            <div className={cn('grid gap-2 px-3 pb-3 pt-8', phone && 'pt-10')}>
                {NOTIFICATIONS.map((notification) => (
                    <article className="rounded-xl border border-white/60 bg-white/88 p-2.5 text-[#303a32] shadow-lg backdrop-blur-xl" key={notification.title}>
                        <header className="grid grid-cols-[auto_1fr_auto] items-center gap-1.5">
                            <img src={jungleBellLogo} alt="" className="size-5 rounded-md" aria-hidden="true"/>
                            <strong className="text-[0.625rem]">Jungle Bell</strong>
                            <small className="text-[0.5625rem] text-[#738076]">{notification.time}</small>
                        </header>
                        <p className="mt-1.5 grid gap-0.5 text-[0.5625rem] leading-snug text-[#68736b]">
                            <strong className="text-[0.6875rem] text-[#303a32]">{notification.title}</strong>

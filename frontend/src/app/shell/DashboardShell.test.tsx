...
describe('dashboard routes', () => {
    ...
    test('personal primary navigation excludes notification and settings utilities', () => {
        ...
        expect(dashboardRouteHref('install')).toBe('#/install');
...
describe('DashboardShell', () => {
    test('renders the shared browser shell, top spacer, and project footer', () => {
        ...
        expect(html).not.toContain(
            'Jungle Bell은 정글 캠퍼스 생활 정보를 한곳에 모아 보여주는 오픈소스 프로젝트입니다.',
        );
    ...
    test('renders notification panel trigger, settings, and the standard sidebar controls', () => {
        ...
        expect(html).toMatch(
            /data-slot="sidebar-menu-badge"[^>]*aria-hidden="true"[^>]*>120<\/div>/u,
        );
        expect(html).toMatch(
            /data-slot="sidebar-menu-badge"[^>]*class="[^"]*top-1\/2![^"]*-translate-y-1\/2!/u,
        );

...
function isPublicCampusDataRequest(url) {
  return url.pathname === '/api/public/status'
...
registerRoute(
  ({request, url, sameOrigin}) => isEligibleGetRequest({request, url, sameOrigin})
    && isPersonalRequest(request, url),
  privateNetworkOnly,
);

...
import { canonicalJsonSha256, sha256Bytes } from "@jungle-bell/backend-common/collection/hash";
import { fetchBinary, fetchJson } from "../clients/http-client";
import { allowedMealMediaHosts, MAX_MEAL_IMAGE_BYTES, rasterImageContentType } from "../clients/meal-media";
import { normalizeLaundry, type LaundryVersion } from "@jungle-bell/backend-common/collection/laundry";
import {
  mealImageExtension,
  normalizeMeals,
  type MealImageAsset,
...
import {
  ...
  minuteEpoch,
  snapshotPath,
} from "@jungle-bell/backend-common/collection/time";
import type {
  CollectAllResult,
  CollectionCommit,
  CollectorOptions,
...
import { SOURCE_NAMES } from "@jungle-bell/backend-common/collection/types";
...
export type CollectionClock = () => Date;
...
export async function collectSources(
  ...
  clock: CollectionClock = () => new Date(),
  ...
  if (requested.has("laundry")) {
    results.push(await collectSource("laundry", options.urls.laundry, storage, options, scheduledAt, clock));
  }
  ...
  if (requested.has("meals-default")) {
    results.push(await collectSource("meals-default", options.urls.mealsDefault, storage, options, scheduledAt, clock));
  }
...
export function collectAll(
  ...
  return collectSources(storage, options, SOURCE_NAMES, scheduledFor, clock);
}

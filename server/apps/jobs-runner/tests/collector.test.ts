...
import { collectAll, collectSources } from "../src/services/source-collection-service";
import type {
  CollectionCommit,
  CollectorOptions,
  SourceName,
...
import type { BinaryObject, CollectorStorage } from "../../../shared/ports/collector-storage";
...
describe("collectAll", () => {
  ...
  it("records failure timing from the injected collection clock", async () => {
    const storage = new MemoryStorage();
    vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response("unavailable", { status: 500 }));
    const instants = [
      new Date("2026-07-17T00:05:00.100Z"),
      new Date("2026-07-17T00:05:00.350Z"),
    ];
    ...
    expect(storage.commits()[0]).toMatchObject({
      state: { lastAttemptAt: "2026-07-17T00:05:00.100Z" },
      observation: {
        collectedAt: "2026-07-17T00:05:00.350Z",
        durationMs: 250,
      },
    });
  });

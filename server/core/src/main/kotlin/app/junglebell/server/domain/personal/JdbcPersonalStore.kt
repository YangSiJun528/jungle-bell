...
@Repository
class JdbcPersonalStore(private val jdbc: JdbcClient) : PersonalStore {
    override fun attendance(userId: UUID): AttendancePreferences = jdbc.sql(
    ...
    override fun saveAttendance(userId: UUID, value: AttendancePreferences, now: Long): AttendancePreferences =
        jdbc.sql(
            """
            ...
            RETURNING enabled, morning_enabled, evening_enabled, morning_start_hour,
                evening_end_hour, morning_interval_minutes, evening_interval_minutes,
                skip_sunday, skip_attendance_date
            """.trimIndent(),
            ...
            .param("skipDate", value.skipAttendanceDate?.let(Date::valueOf)).param("now", now)
            .query(::attendanceRow).single()

    override fun meal(userId: UUID): MealPreferences = jdbc.sql(
    ...
    override fun saveMeal(userId: UUID, value: MealPreferencesInput, now: Long): MealPreferences =
        jdbc.sql(
            """
            ...
            ON CONFLICT (user_id) DO UPDATE SET enabled = EXCLUDED.enabled,
                ...
                updated_at_epoch_ms = EXCLUDED.updated_at_epoch_ms
            RETURNING enabled, lunch, dinner, updated_at_epoch_ms
            """.trimIndent(),
        ).param("userId", userId).param("enabled", value.enabled).param("lunch", value.lunch)
            .param("dinner", value.dinner).param("now", now).query(::mealRow).single()

    override fun watches(userId: UUID): List<LaundryWatch> = jdbc.sql(
    ...
    override fun createWatch(userId: UUID, watch: LaundryWatch): Boolean = jdbc.sql(
            """
    ...
    override fun cancelWatch(userId: UUID, id: String, now: Long): Boolean = jdbc.sql(
        """
    ...
    private fun attendanceRow(row: java.sql.ResultSet, index: Int) = AttendancePreferences(
        row.getBoolean("enabled"), row.getBoolean("morning_enabled"), row.getBoolean("evening_enabled"),
        row.getInt("morning_start_hour"), row.getInt("evening_end_hour"),
        row.getInt("morning_interval_minutes"), row.getInt("evening_interval_minutes"),
        row.getBoolean("skip_sunday"), row.getDate("skip_attendance_date")?.toLocalDate()?.toString(),
    )

    private fun mealRow(row: java.sql.ResultSet, index: Int) = MealPreferences(
        row.getBoolean("enabled"), row.getBoolean("lunch"), row.getBoolean("dinner"),
        row.getLong("updated_at_epoch_ms"),
    )
}

...
import { runLaundryLifecycle } from "../src/application/laundry-lifecycle";
import { planLaundryTransition } from "../src/domain/laundry-notifications";
import {
  DESKTOP_ENROLLMENT_POLICY,
  MANUAL_PAIRING_CLAIM_POLICY,
  PAIRING_CREATION_POLICY,
} from "../src/domain/enrollment-policy";
import { PAIRING_TTL_MS } from "../src/renewal/service";
...
function sqliteD1(database: DatabaseSync, onBatch?: (statementCount: number) => void): D1Database {
  return {
    prepare(sql: string) {
      return {
...
describe("D1RenewalStore laundry lifecycle", () => {
  ...
  it("atomically claims the FIFO turn, inserts its notification, and fans out to the active desktop", async () => {
    ...
    try {
      ...
      database.prepare(`INSERT INTO desktop_device (installation_id, user_id, created_at_epoch_ms,
        last_seen_at_epoch_ms, lms_session_state, app_version)
        VALUES ('desktop-1', 'user-1', ?, ?, 'connected', '0.5.0')`).run(now, now);
      database.prepare(`INSERT INTO app_session (id, user_id, installation_id, kind, label, token_sha256,
        created_at_epoch_ms, expires_at_epoch_ms, last_seen_at_epoch_ms, revoked_at_epoch_ms, source_pairing_id)
      ...
      database.prepare(`INSERT INTO laundry_queue_entry
        ...
        VALUES ('queue-1', 'user-1', NULL, 'washer', 'waiting', ?, NULL)`).run(now - 1_000);
      ...
      expect(database.prepare("SELECT status, left_at_epoch_ms FROM laundry_queue_entry WHERE id = 'queue-1'").get())
        .toEqual({ status: "claimed", left_at_epoch_ms: now });
      const notification = database.prepare("SELECT id, user_id, source_event_id FROM notification").get() as {
        id: string; user_id: string; source_event_id: string;
      };
      ...
      expect(database.prepare(`SELECT target_kind, target_id FROM notification_delivery WHERE notification_id = ?`).all(notification.id))
        .toEqual([{ target_kind: "desktop", target_id: "desktop-1" }]);
      expect(database.prepare("SELECT source_id FROM laundry_lifecycle_processing WHERE source_id = 'event-1'").all())
        .toEqual([{ source_id: "event-1" }]);

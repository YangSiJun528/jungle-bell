...
import app.junglebell.server.domain.publicapi.MealImage
import app.junglebell.server.domain.publicapi.MealPost
import app.junglebell.server.domain.publicapi.PublicDataStore
import app.junglebell.server.domain.publicapi.StoredMealImage
import app.junglebell.server.domain.publicapi.StoredMealPublication
...
class MealNormalizer(
    ...
    fun normalize(root: JsonNode, observedAt: Instant): List<StoredMealPublication> {
    ...
    private fun normalizePost(raw: JsonNode, observedAt: Instant): StoredMealPublication {
        ...
        return StoredMealPublication(post, images, sourceWeekKey(title, updatedAt ?: observedAt))
    }
    ...
    private fun normalizeImage(postId: String, position: Int, media: JsonNode): StoredMealImage? {
        ...
        val sourceUrl = (media.get("xlarge_url") ?: media.get("url"))?.asString()?.replace("http://", "https://")
            ?: return null
        store.mealImage(postId, mediaId)?.takeIf { it.sourceUrl == sourceUrl }?.let { return it }

import { ATTENDANCE_CLIENT_CLOCK_SKEW_MS, DESKTOP_ONLINE_WINDOW_MS } from "../renewal/attendance-policy";
import { RenewalError, type Principal } from "../renewal/service";
...
export async function readAttendance(store: RenewalStore, userId: string, nowEpochMs: number): Promise<AttendanceEnvelope> {
  return attendanceEnvelope(await store.getLatestAttendanceSnapshot(userId), nowEpochMs);
}

export async function readMobileAttendance(store: RenewalStore, userId: string, nowEpochMs: number) {

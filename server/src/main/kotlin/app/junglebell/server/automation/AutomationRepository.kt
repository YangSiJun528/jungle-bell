...
@Repository
class AutomationRepository(private val jdbc: JdbcClient) {
    fun tryAcquireLease(name: String, now: Long, durationMs: Long, token: String): Boolean = jdbc.sql(
        """
        ...
        .param("expiresBefore", now - durationMs).update() == 1

    fun attendancePreferences(): List<AttendanceCandidate> = jdbc.sql(
        """
    ...
    fun desktopStates(userId: UUID): List<DesktopState> = jdbc.sql(
        """
    ...
    fun recentMealPublications(since: Instant): List<MealPublication> = jdbc.sql(
        """
    ...
    fun activeLaundryWatches(): List<ActiveLaundryWatch> = jdbc.sql(
        """
    ...
    fun completeLaundryWatch(id: String, now: Long): Boolean = jdbc.sql(
        """
    ...
    fun claimPushDeliveries(now: Long, leaseToken: String, limit: Int): List<PushDelivery> = jdbc.sql(
        """
        WITH candidates AS (
    ...
    fun settlePush(
        ...
        .param("subscriptionId", delivery.subscriptionId).param("leaseToken", leaseToken).update() == 1

    fun revokePush(subscriptionId: String, now: Long) {
        jdbc.sql(
            "UPDATE push_subscription SET revoked_at_epoch_ms = :now WHERE id = :id AND revoked_at_epoch_ms IS NULL",
        ).param("now", now).param("id", subscriptionId).update()
    }

    fun runHousekeeping(now: Long): Map<String, Int> = linkedMapOf(

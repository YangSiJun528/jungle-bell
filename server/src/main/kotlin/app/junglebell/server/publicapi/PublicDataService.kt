...
class PublicDataService(
    ...
    fun health(): Pair<PublicHealth, HttpStatus> {
        ...
        val states = repository.sourceStates()
    ...
    @Transactional(readOnly = true)
    fun status(): PublicStatus = PublicStatus(cacheSlice(clock.instant()).toString(), repository.sourceStates())

    @Transactional(readOnly = true)
    fun laundryHead(): SourceState = repository.sourceState("laundry")
        ?: throw ApiException("NO_DATA", HttpStatus.SERVICE_UNAVAILABLE)

    @Transactional(readOnly = true)
    fun laundry(): PublicLaundrySnapshot {
        val version = repository.latestLaundryVersion()
            ?: throw ApiException("NO_DATA", HttpStatus.SERVICE_UNAVAILABLE)
        return project(version, repository.sourceState("laundry"), cacheSlice(clock.instant()), false)
    }

    @Transactional(readOnly = true)
    fun laundryVersion(sha: String): LaundryVersion = repository.laundryVersion(sha)
    ...
    fun laundryAt(minute: String): MinuteLaundryResponse {
        ...
        val observation = repository.observation(instant.epochSecond / 60)
            ?: throw ApiException("OBSERVATION_NOT_FOUND", HttpStatus.NOT_FOUND)
        val data = observation.versionSha?.let(repository::laundryVersion)?.let {
    ...
    @Transactional(readOnly = true)
    fun laundryEvents(since: Instant?, limit: Int) = mapOf("events" to repository.laundryEvents(since, limit))
    ...
    fun asset(sha: String, extension: String): StoredAsset {
        val asset = repository.asset(sha) ?: throw ApiException("ASSET_NOT_FOUND", HttpStatus.NOT_FOUND)

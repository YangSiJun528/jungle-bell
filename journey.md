...
(또, 같은 반 분들의 도움으로 게임랩, 테크랩 채널에 간접적으로 홍보를 할 수 있었는데, 여기서 실명을 언급할 수는 없지만 이 기회에 감사하다는 말을 하고 싶다.)
...
사람들이 많이 지나다니는 곳에 홍보물을 붙였다. 층별 복도, 세탁실, 흡연실(오래 대기하고 있으니 특별히 2장) 등등
...
이전에 느꼈던 설치가 여러운 문제를 해결하기 위해, 더 친숙할 방식인 설치 스크립트를 만들었다. 범용 서비스로 쓸 수 있게 [`install.sijun-yang.com`](https://github.com/YangSiJun528/install.sijun-yang.com)로 만들었지만 딱히 쓰일데는 없었다. (Cloudflare Workers에서 Hono를 사용해 돌아간다.)

앱에서 GitHub로 피드백을 보낼 수 있게 하고(슬랙, 이메일로도 보내게 했다), 실제로 얼마나 쓰는지 궁금해서 분석용 통계도 수집하기 시작했다. (Rust SDK 제공하고 어느정도 무료인게 PostHog만 있어서 이걸 사용했는데, 생각보다 유명한 서비스였음.)
...
초반 기술 스택은 Hono 기반의 Cloudflare Workers를 사용했다. 그런데 cron으로 식사 정보를 긁어오는 작업이 수십초씩 걸리는데, Workers는 CPU 시간을 1초 이하(정확한 시간은 모름)만 사용할 수 있어 요청을 정상적으로 처리하지 못한다.
처음에는 그 작업만 따로 돌리려고 오라클 클라우드(OCI)에 가입해서 프리티어 인스턴스를 만들어 사용했다. 

(오라클은 프리티어 인스턴스를 넉넉하게 주는 편이다. 최대 2 OCPU(AWS에선 4 vCPU에 해당) 및 12GB RAM을 제공한다. 다만 몇 가지 서비스의 프리티어 사용량은 가입 시 설정한 초기 리전에서만 유효하므로 주의해야한다. 고사양의 프리티어 컴퓨터는 리전별로 개수가 제한되어있어 쉽게 생성할 수 없는데, 이 때문에 넉넉하다는 싱가폴을 선택했다. 정글벨도 싱가폴에 위치한 인스턴스에서 동작하다보니 물리적 거리가 있어서 뭘 해도 100ms 이상의 레이턴시를 보여준다는 단점이 있다...)

그렇게 필요한 부분만 옮겨서 쓰다가 보니, 너무 파편화된 거 같아 결국 서버를 익숙한 Spring Boot 기반으로 변경하고 오라클 Compute(AWS의 EC2와 비슷) 하나에 컨테이너 환경으로 격리해서 돌아가도록 한 서버로 합쳤다.

Cloudflare D1, R2 환경도 이관하고, API 서버와 수집 작업, DB까지 한 컴퓨터에서 돌고 있다.
...
현재는 위 그림처럼 OCI(Oracle Cloud Infrastructure)의 컴퓨팅 자원 하나 안에서 처리하고 있다. (배포는 Tailscale로 접근해서 하고, 외부 접근은 Cloudflare tunnel로 HTTPS인 443 포트만 열어둔다.)

웹 화면에 필요한 정적 파일과 식단 이미지를 서버에서 내려주지만, React 파일이나 이미지같은 공개 에셋은 사용자마다 달라지는 내용이 없어서 캐시를 적극적으로 활용하니 크게 속도 문제를 느끼지 않고 있다.

## 현황

SW-AI Lab 13기 분들이 들어오고 나선 크게 홍보하지 않았는데, 어느새 다시 많은 사용자가 사용하고 있다. 

필요한 기능은 이제 거의 다 만든 것 같아서, 당분간 기능을 크게 추가할 생각은 없지만 관리는 꾸준히 하는 중이다.

최근에는 FE/BE/QA 담당/검증 역할의 에이전트들을 만들어서 실행하는 방식을 시도해보았는데 지침을 조금 더 대충해도 잘 동작하는듯 하다. 특히 QA 에이전트한테 화면 포커스를 가져가지 말라고 지침해서 이전에 종종 발생하던 테스트하겠다고 화면이나 커서를 뺏어가는 문제가 없어져서 좋다.

## 여담

#### 1. 이스터에그 출현

이번에 크래프톤 게임랩 3기에서 출시해서 좋은 반응을 얻은 '노 리절트 파운드'라는 게임에 이스터에그(?)로 나왔다 게임 속 커뮤니티에 JungleBell이라는 사용자 이름으로 등장한다. 

바이럴이 되서 여러 유튜버가 하는 게임에 나와 관련된 이스터그가 들어있다는게 좀 묘한 기분이였다.
...
누군가 이 프로젝트를 포크해서 기능을 확장하고, 안드로이드용 위젯을 만들었더라. 내가 만든 걸 누군가 포크해서 가져다 쓰는 건 처음이라 신기한 경험이였다.

앞으로도 오픈소스 활동을 하고 프로젝트를 만들면서 이런 일이 더 자주 생겼으면 좋겠다고 생각한다. 

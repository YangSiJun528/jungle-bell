...
test('개인 설정 초기 상태는 공개 화면에 데이터를 추측하지 않는다', () => {
    const state = initialDashboardPersonalState();
    assert.equal(state.attendancePreferences, null);
    assert.equal(state.mealPreferences, null);
    assert.deepEqual(state.laundryWatches, []);
    assert.deepEqual(state.laundryQueue, []);
    assert.equal(state.personalControlsState, 'loading');
});

...
describe('loadImageWithTimeout', () => {
    test('로드와 디코딩이 끝난 이미지를 반환한다', async () => {
        ...
        const loading = loadImageWithTimeout(image, 'https://example.com/v1/assets/meal.png', 1_000);
        ...
        await loading;
        assert.equal(image.src, 'https://example.com/v1/assets/meal.png');
    ...
    test('이미지 오류를 호출자에게 전달한다', async () => {
        const image = new FakeImage();
        const loading = loadImageWithTimeout(image, 'https://example.com/v1/assets/missing.png', 1_000);
    ...
    test('제한 시간 안에 끝나지 않으면 실패한다', async () => {
        vi.useFakeTimers();
        const image = new FakeImage();
        const loading = loadImageWithTimeout(image, 'https://example.com/v1/assets/slow.png', 15_000);

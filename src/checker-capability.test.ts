...
test('구형 출석 WebView injection과 capability는 제거한다', () => {
    assert.equal(existsSync(new URL('./injected/attendance.ts', import.meta.url)), false);
    assert.equal(existsSync(new URL('./injected/attendance-decision.ts', import.meta.url)), false);
    assert.equal(existsSync(new URL('../src-tauri/capabilities/attendance.json', import.meta.url)), false);
});

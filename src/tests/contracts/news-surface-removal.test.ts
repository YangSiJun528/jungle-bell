...
test('React 대시보드와 Tauri IPC에는 제거된 앱 소식 기능이 남지 않는다', () => {
    ...
    assert.equal(existsSync(new URL('../src-tauri/src/news.rs', srcRoot)), false);
    assert.equal(existsSync(new URL('../.github/workflows/publish-news.yml', srcRoot)), false);
});

test('소식 대신 Markdown 콘텐츠를 src/site 아래의 정적 Astro 블로그로 관리한다', () => {
    assert.equal(existsSync(new URL('./site/content/posts/welcome.md', srcRoot)), true);
    assert.equal(existsSync(new URL('./site/pages/blog/index.astro', srcRoot)), true);
    assert.equal(existsSync(new URL('./site/pages/blog/posts/[slug].astro', srcRoot)), true);
    assert.equal(existsSync(new URL('../site/package.json', srcRoot)), false);
    assert.equal(existsSync(new URL('../site/astro.config.mjs', srcRoot)), false);
    assert.equal(existsSync(new URL('../site/src/pages/blog/index.astro', srcRoot)), false);
...
test('정적 블로그는 React 통합과 hydration directive를 사용하지 않는다', () => {
    ...
    const siteDirectory = fileURLToPath(new URL('./site/', srcRoot));

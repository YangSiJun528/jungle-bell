...
import type {LaundryCapacitySnapshot} from '@/dashboard-model';
...
describe('capacityCards', () => {
    it('서버가 신뢰 가능하다고 표시한 수치만 시작 가능 횟수로 노출한다', () => {
        expect(capacityCards(capacity, true)).toEqual([
            expect.objectContaining({access: 'men', count: 1, status: 'available'}),
            expect.objectContaining({access: 'women', count: null, status: 'checking'}),
        ]);
    });

import type {DesktopHttpSessionBootstrap, NativeBridge} from './native-bridge';
...
export interface DesktopHttpSessionManager {
    getSessionLease(): Promise<DesktopHttpSessionLease>;
    refreshAfterUnauthorized(rejectedLease: DesktopHttpSessionLease): Promise<DesktopHttpSessionLease>;
    assertCurrent(lease: DesktopHttpSessionLease): void;
    clear(): void;
}

export interface DesktopHttpSessionLease {
...
export function createDesktopHttpSessionManager(options: {
    ...
    const assertCurrent = (lease: DesktopHttpSessionLease): void => {
        if (lease.generation !== generation) {
            throw new Error('DESKTOP_HTTP_SESSION_INVALIDATED');
        }
    };

    const sessionLease = async (): Promise<DesktopHttpSessionLease> => {

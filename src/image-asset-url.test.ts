...
test('이미지 자산 URL은 HTTPS 또는 로컬 HTTP의 assets 경로만 허용한다', () => {
    assert.equal(isSafeImageAssetUrl('https://api.example.com/v1/assets/menu.png'), true);
    assert.equal(isSafeImageAssetUrl('http://127.0.0.1:43120/v1/assets/menu.png'), true);
    assert.equal(isSafeImageAssetUrl('http://example.com/v1/assets/menu.png'), false);

...
도구 폴더의 `observations`에 검토 이력을 남기려면 다음처럼 실행한다.

```bash
npm run analyze -- --snapshot-root observations
```
...
```text
tools/campus-observer/output/report.json         현재 결과
tools/campus-observer/observations/logs/         매 실행 기준 스냅샷, Git 제외
tools/campus-observer/observations/changes/      변경이 있을 때만 생성되는 검토 파일
```

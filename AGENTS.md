...
메인은 조사·작업 분해·위임·결과 통합·검증 관리를 맡습니다. 전문 역할의 담당 범위에 속하는 구현·수정은
[.codex/agents](.codex/agents/)에서 필요한 역할을 골라 내장 `spawn_agent`의 `agent_type`으로
위임합니다. 이 범위에서 메인이 직접 수정할 수 있는 예외는 의미·로직·동작·계약·설정·산출 결과를 바꾸지 않는
문서·주석의 단순 오타·서식 수정뿐입니다. 줄 수·파일 수·예상 작업 시간이 작아도 로직,
사용자에게 표시되는 값·산출 결과 또는 검증·완료 판정 규칙을 바꾸면 위임합니다.
새 담당자는 `fork_turns="none"`으로 목표·수정 범위·완료 조건·협업 상대와 해당 폴더 지침 위치를 받습니다.
Frontend·Desktop·Server는 구현과 자체 검증을 맡습니다. 동작·계약·상태·실행 설정 또는
검증·완료 판정 규칙을 바꾸면 구현에 참여하지 않은 Reviewer의 독립 검토를 거칩니다.
독립 검토는 구현 위임을 대신하지 않습니다. Visual QA는 실제 화면·OS 동작을
관찰해야 하는 조건을 맡고, 전체 UI 확인은 큰 기능 완료·최종 릴리스에 수행합니다.
...
담당자는 변경 내용과 완료 조건별 판정(통과·실패·미검증), 실행·관찰 근거와 남은 문제를 반환합니다.
요구사항의 모호함·실패·막힘은 메인에
알리고, 메인은 결과 통합과 Goal 진행을 관리합니다. 작업이 없을 때 반복 실행하거나
미해결 상태로 무한 대기하지 않습니다.

## QA

모든 작업은 완료 조건과 검증 방법을 정하고, 필수 조건의 실패·미검증이 남으면 완료로 보고하지 않습니다.
일반 작업은 최소 스모크와 변경 범위를 확인하고, 순수 문서 변경은 문서 검사로 확인합니다.
리뷰 지적을 수정하면 해당 조건을 재검증하고, 여러 영역을 연결한 변경은 최종 조합도 확인합니다.
상세 절차는 [메인 작업 안내](docs/guide-codex-main.md#작업별-검증과-완료-판정)를 따릅니다. 릴리스 전에는

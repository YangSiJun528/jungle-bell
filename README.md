...
PC 화면의 QR은 설치 파일이 아니라 **모바일 설치 안내와 연결 인계(handoff)를 시작하는
링크**입니다. 먼저 PWA를 설치하고, 설치한 앱을 열면 PC 연결 요청이 자동으로 이어집니다.

1. PC 앱을 실행하고 대시보드에서 **공식 정글캠퍼스 열기**를 눌러 로그인합니다.
2. **설정 → 기기 연결**에서 **휴대폰 설정 QR 만들기**를 누릅니다.
3. 휴대폰 카메라로 QR을 스캔합니다. 앱 설치 안내의 모바일 영역이 열리고 연결 정보가
   설치할 앱으로 인계될 준비를 마칩니다.
4. iPhone은 **홈 화면에 추가**, Android는 **앱 설치**를 선택한 뒤 홈 화면의 Jungle
   Bell 아이콘을 직접 엽니다.
5. PWA가 연결 요청을 자동으로 시작하면 휴대폰의 4자리 확인 번호와 기기명을 PC에서
   대조한 뒤 **이 휴대폰 승인**을 누릅니다.
6. 표시되는 **알림 확인 (선택)**에서 테스트하거나 **나중에**를 누릅니다. 건너뛰어도
   앱 사용에는 영향이 없으며 알림 센터에서 다시 테스트할 수 있습니다.

연결 코드, QR, 설치 인계 정보는 10분 동안 유효합니다. QR의 일회용 값은 주소
표시줄에서 즉시 제거되고 브라우저 저장소에 남지 않습니다. QR 자체가 앱을 설치하거나
설치한 앱을 자동으로 열 수는 없으므로 설치 후 홈 화면 아이콘을 한 번 열어야 합니다.
모바일 session은 연결 후 최대 30일 유지되며 PC 앱에서 언제든 해제할 수 있습니다.

브라우저가 설치 인계 정보를 PWA에 전달하지 못했거나 정보가 만료되면 연결 화면이
표시됩니다. 이 경우 PC에서 새 연결을 만들고 설치한 PWA 안에서 10자리 코드를 입력하면
됩니다.

...
4. If the user only asked for a draft message, stop after returning the draft.
5. If the requested commit scope is ambiguous, unrelated changes are mixed in, or sensitive files are present, ask one concise question before committing. Otherwise:
   - In Codex, commit directly; the user already requested a commit.
   - In runtimes that require explicit confirmation and provide `AskUserQuestion`, use the approval loop below.
6. Stage only the intended files by explicit path if they are not already staged. Avoid broad staging commands.
7. Commit with `git commit -m`. Use multiple `-m` arguments when a body is needed:

```bash
git commit -m "<type>(<scope>): <subject>" -m "<body>"
```

If there is no body, use a single-line `git commit -m "<type>(<scope>): <subject>"`.

8. Confirm success with `git status` afterward.

## Legacy approval loop

Use this only in runtimes that provide `AskUserQuestion` and require explicit confirmation before committing.

Question setup (`header`: "커밋 승인", `multiSelect: false`):

- `커밋` — commit with the current draft as-is
- `취소` — abort without committing
- **(기타)** — free-text feedback. If the user enters feedback, revise the draft, print the new draft, and ask again.

Use a question like "이 메시지로 커밋할까요? 고칠 점이 있으면 'Type something'에 피드백을 적어 주세요." Loop until `커밋` or `취소` is selected.
...
- Use the current runtime's user-input style only when a decision is actually required.

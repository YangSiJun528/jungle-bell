...
정식 릴리스는 이미 공개된 모든 정식 버전보다 높아야 합니다. 프리릴리스는 공개된 정식
버전과 같은 `X.Y.Z` 계열의 프리릴리스보다 높아야 합니다. 다른 계열의 향후 베타는
현재 계열의 정식 출시를 막지 않습니다. 비교는 태그의 SemVer 순서를 따릅니다.
워크플로는 빌드 전과 공개 승인 후에 GitHub 이력을 다시 조회해 검사합니다. 각 업로드
잡도 초안 상태와 릴리스 ID를 다시 확인하므로 공개된 릴리스의 개별 잡 재실행은 차단됩니다.
실패한 미공개 초안의 같은 버전 재시도는 허용합니다.

릴리스 요청을 받은 에이전트는 초안과 빌드를 준비한 뒤 해당 Actions 실행 링크를 제공합니다.
공개 승인은 GitHub의 `desktop-release` 환경에서만 받으며 대화에서 중복 확인하지 않습니다.
버전 정책 회귀 검사는 `node --test scripts/test-release-policy.mjs`로 실행하며 CI에도 포함됩니다.
